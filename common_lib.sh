...
# reference: https://github.com/natelandau/shell-scripts
# Set Base Variables
# ----------------------
...
function must_have_folder() {
	if [ ! -d $1 ]; then
    error "$1 not exist. quit now."
    exit 1;
	fi
}
...
function must_have_command() {
  eval $1
  if [ $? -ne 0 ]; then
    error "$1 not executed successfully. quit now."
    exit 1;
  fi
}
...
# Options and Usage
# -----------------------------------
function args_checking() {
 if [ $# -ne $ARGS_NUMBER  ] ; then
    error "wrong arguments. Please check usage via option --help"
    exit 1;
fi
}

function usage() {
  echo -n "${scriptName} [OPTION] ${script_usage}...
 ${bold}Options:${reset}
  -v, --verbose     Output more information. (Items echoed to 'verbose')
  -h, --help        Display this help and exit
      --version     Output version information and exit
${script_usage_example} 
${script_purpose}     
"
}
